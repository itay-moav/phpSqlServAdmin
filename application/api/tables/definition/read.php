...
/**
 * curl http://vulcan/tables/definition/connectionName/itay/dbname/a/table/dbo.smick
 * 
 * @author itay
 * @date 2022-11-24
 */
class TablesDefinitionRead extends \Talis\Chain\aFilteredValidatedChainLink{
 ...
 * @date 2022-11-24
 */
class ConstructCreateStatement extends \lib\Database\ChainWithConnection{
    /**
     * {@inheritDoc}
...
SELECT @SQL = 'CREATE TABLE ' + @object_name  + '(' + {$newline} + STUFF((
         ...
         {$newline} + 'CREATE' + CASE WHEN i.is_unique = 1 THEN ' UNIQUE' ELSE '' END 
                ...
                + ISNULL({$newline} + 'INCLUDE (' + 
                    ...
                    WHERE c.is_included_column = 1
                        AND c.index_id = i.index_id
                    FOR XML PATH(''), TYPE).value('.', 'NVARCHAR(MAX)'), 1, 2, '') + ')', '')  + {$newline}
